...
class DocumentSynthesizer(object):
    ...
    def generate_base_image(self, text: str, font: ImageFont, spacing: int):
        """
        Generates a base image for the text, with a random size and random rotation
        """
        line_length_in_chars, number_of_lines = self.evaluate_font_size(font, text, spacing)


        if self.rng.rand() >= self.arguments.overflow_probability:
        ...
        np_image = self.embed_image(np_image)
        return np_image

    def embed_image(self, np_image: np.ndarray, add_frame=True) -> np.ndarray:
        """
