...
class SyntheticDatasetTorch(IterableDataset):
    """
    ...
    def __init__(self, text_dataset,
        ...
        self.step = 0
        self.max_step = None
        self.randomness_intensity_update_interval = -1
        self.warmup_steps = 0
    ...
    def __iter__(self):
        ...
        """
        if self.overfit_examples is None:
            info = torch.utils.data.get_worker_info()
            self.rng = np.random.RandomState(self.step + info.id)
            logging.info(f"randomizing dataset with worker id={info.id if info else 0} and step={self.step}")
        while True:
            if self.overfit_examples is not None and self.step >= 10:  # then we will return one of 10 examples
                yield self.overfit_examples[np.random.randint(0, len(self.overfit_examples))]
            else:
                text_index = self.rng.randint(0, len(self.text_dataset))
                text = self.text_dataset[text_index]["text"]
                ...
                if self.step < self.warmup_steps // 2:
                ...
                if (self.randomness_intensity_update_interval > 0)\
                        and (self.step % self.randomness_intensity_update_interval == 0):
